...
export function Task({ item, onRemoveTask, setTaskToUpdate, labels, columns }) {
    ...
    const { attributes, listeners,
        setNodeRef, transform, transition, isDragging } = useSortable({ id: item.id })
    ...
    return (
        <div className={`table-row ${isDragging ? 'task-draging' : ""}`} ref={setNodeRef} {...attributes} style={style}>
